...
class BufferAttributesCalculator:
    ...
    def calculate_morning_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2013-01-01 06:00:00':'2013-01-01 11:30:00']
    ...
    def calculate_lunch_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2013-01-01 11:30:00':'2013-01-01 14:30:00']
        max_values = selected_columns.max(axis=1)
        # Calculate the average of the max values
        return max_values.mean()
    
    def calculate_afternoon_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2013-01-01 14:30:00':'2013-01-01 18:00:00']
        max_values = selected_columns.max(axis=1)
        # Calculate the average of the max values
        return max_values.mean()
    
    def calculate_evening_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2013-01-01 18:00:00':'2013-01-01 21:30:00']
        max_values = selected_columns.max(axis=1)
        # Calculate the average of the max values
        return max_values.mean()
    
    def calculate_night_peak(self, buffer_data):
        columns_midnight_to_six = buffer_data.loc[:, '2013-01-01 00:00:00':'2013-01-01 06:00:00']
        columns_night = buffer_data.loc[:, '2013-01-01 21:30:00':'2013-01-01 23:30:00']
    ...
    def make_buffer_attributes(self, result_path, meter_ids=None, start_date=datetime.date(2013, 1, 1)):
    ...
    def make_buffer_attributes_all(self, result_path, meter_ids=None, start_date=datetime.date(2013, 1, 1)):
        if Path(result_path).exists():
            return
...
if __name__ == "__main__":
    ...
    data = pd.read_pickle(os.path.join(london_data_dir,'daily_data_df_filtered.pkl'))

    # Example usage:
    for i in range(2,13):
        print(f"Starting month {i}")
        start_date = datetime.date(2013, i, 1)
        calculator = BufferAttributesCalculator(data)
        calculator.make_buffer_attributes_all(os.path.join(london_data_dir,f'buffer_attributes_month_{i}_catch22_df.pkl'),start_date=start_date)
