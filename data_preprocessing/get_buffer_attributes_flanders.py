...
class BufferAttributesCalculator:
    ...
    def calculate_morning_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2021-01-01 06:00:00':'2021-01-01 11:30:00']
    ...
    def calculate_lunch_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2021-01-01 11:30:00':'2021-01-01 14:30:00']
        max_values = selected_columns.max(axis=1)
        # Calculate the average of the max values
        return max_values.mean()
    
    def calculate_afternoon_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2021-01-01 14:30:00':'2021-01-01 18:00:00']
        max_values = selected_columns.max(axis=1)
        # Calculate the average of the max values
        return max_values.mean()
    
    def calculate_evening_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2021-01-01 18:00:00':'2021-01-01 21:30:00']
        max_values = selected_columns.max(axis=1)
        # Calculate the average of the max values
        return max_values.mean()
    
    def calculate_night_peak(self, buffer_data):
        columns_midnight_to_six = buffer_data.loc[:, '2021-01-01 00:00:00':'2021-01-01 06:00:00']
        columns_night = buffer_data.loc[:, '2021-01-01 21:30:00':'2021-01-01 23:30:00']
    ...
    def make_buffer_attributes(self, result_path, meter_ids=None, start_date=datetime.date(2021, 1, 1)):
    ...
    def make_buffer_attributes_all(self, result_path, meter_ids=None, start_date=datetime.date(2021, 1, 1)):
        if Path(result_path).exists():
            return
        ...
        buffer_attributes_df = pd.DataFrame(columns=columns_all + columns_weekday + columns_weekend)
        buffer_attributes_df.index.name = "meterID"
        ...
        for meter_id in meter_ids:
            ...
            row_values_combined = row_values_all + row_values_weekday + row_values_weekend
...
if __name__ == "__main__":
    ...
    data_dir = os.path.join(current_dir, 'Data/Flanders_dataset/preprocces')
    
    data = pd.read_pickle(os.path.join(data_dir,'flanderstimeseries.pkl'))
    data.head()

    # Example usage:
    for i in range(1,13):
        print(f"Starting month {i}")
        start_date = datetime.date(2021, i, 1)
        calculator = BufferAttributesCalculator(data)
        calculator.make_buffer_attributes_all(os.path.join(data_dir,f'buffer_attributes_month_{i}_all_df.pkl'),start_date=start_date)
    print("done")
