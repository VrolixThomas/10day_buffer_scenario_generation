...
class BufferAttributesCalculator:
    ...
    def calculate_morning_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2022-01-01 06:00:00':'2022-01-01 11:30:00']
    ...
    def calculate_lunch_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2022-01-01 11:30:00':'2022-01-01 14:30:00']
        max_values = selected_columns.max(axis=1)
        # Calculate the average of the max values
        return max_values.mean()
    
    def calculate_afternoon_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2022-01-01 14:30:00':'2022-01-01 18:00:00']
        max_values = selected_columns.max(axis=1)
        # Calculate the average of the max values
        return max_values.mean()
    
    def calculate_evening_peak(self, buffer_data):
        selected_columns = buffer_data.loc[:, '2022-01-01 18:00:00':'2022-01-01 21:30:00']
        max_values = selected_columns.max(axis=1)
        # Calculate the average of the max values
        return max_values.mean()
    
    def calculate_night_peak(self, buffer_data):
        columns_midnight_to_six = buffer_data.loc[:, '2022-01-01 00:00:00':'2022-01-01 06:00:00']
        columns_night = buffer_data.loc[:, '2022-01-01 21:30:00':'2022-01-01 23:30:00']
    ...
    def make_buffer_attributes_all(self, result_path, meter_ids=None, start_date=datetime.date(2022, 1, 1)):
        ...
        for meter_id in meter_ids:
            ...
            buffer_data_catch22 = pycatch22.catch22_all(values_list)['values']
...
if __name__ == "__main__":
    ...
    data_dir = os.path.join(current_dir, 'Data/Flanders_dataset_big/preprocces')
    # CHANGE THIS TO THE PATH OF YOUR DATA
    data = pd.read_pickle('Data/Flanders_dataset_big/preprocess/flanderstimeseries.pkl')
    data.head()
    ...
    for i in range(2,13):
        ...
        start_date = datetime.date(2022, i, 1)
        calculator = BufferAttributesCalculator(data)
        # CHANGE THIS TO THE PATH OF YOUR DATA
        calculator.make_buffer_attributes_all(os.path.join('/Data/Flanders_dataset_big/preprocess',f'buffer_attributes_month_{i}_all_df.pkl'),start_date=start_date)
    print("done")
