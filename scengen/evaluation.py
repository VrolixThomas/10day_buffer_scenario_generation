...
def calculate_energy_scores_for_index_samples(train_timeseries, all_samples, correct_profiles):
    ...
    for i in range(all_samples.shape[0]):
        ...
        correct_profile = correct_profiles.iloc[i]
        samples_ts = train_timeseries.iloc[samples_idx]
...
def calculate_energy_score(samples, correct_profile):
    ...
    first_term = np.sum(
        euclidean_distances(samples, correct_profile.to_numpy().reshape((1, -1))).squeeze()
    )
